...
### Added
- **i18n support (EN/RU):** `/language` command to switch UI language; all
  keyboards, formatters, handlers, middlewares, and precheck messages are
  localized via `t(key, lang)` helper.
...
- **Scheduled purge:** `/tasks/purge` endpoint soft-deletes old records.
